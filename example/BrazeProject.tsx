...
export const BrazeProject = (): ReactElement => {
  ...
  useEffect(() => {
    console.log(
      `Initializing Braze SDK with API key: ${defaultApiKey} and endpoint: ${defaultEndpoint}`,
    );
    ...
    Braze.getInitialPushPayload(pushPayload => {
      if (pushPayload) {
        ...
        const initialURL = pushPayload.url;
        if (initialURL) {
    ...
    // Expo-specific: surface foreground notification events from expo-notifications.
    const notificationListener = Notifications.addNotificationReceivedListener(
      notification => {
        console.log(
          `expo-notifications addNotificationReceivedListener triggered:\n${JSON.stringify(notification)}`,
        );
      },
    );
    ...
    const responseListener =
      Notifications.addNotificationResponseReceivedListener(response => {
        console.log(
          `expo-notifications addNotificationResponseReceivedListener triggered:\n${JSON.stringify(response)}`,
        );
      });
